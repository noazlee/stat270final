...
```{r}
library(poLCA)
f <- cbind(Gender, Breastfeeding, Varicella, Initial_Symptom, Mono_or_Polysymptomatic, Oligoclonal_Bands, LLSSEP, ULSSEP, VEP, BAEP, Periventricular_MRI, Cortical_MRI, Infratentorial_MRI, Spinal_Cord_MRI)~1
m1 <- poLCA(f, data=df2, nclass=2, graphs=TRUE, na.rm=TRUE)
```

```{r}
m2 <- poLCA(f, data=df2, nclass=3, graphs=TRUE, na.rm=TRUE)
```
```{r}
m3 <- poLCA(f, data=df2, nclass=4, graphs=TRUE, na.rm=TRUE)
```
